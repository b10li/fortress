...
# (Line 2) function getVxy(unitEpd)
# (Line 3) {
@EUDFunc
def f_getVxy(unitEpd):
    # (Line 4) const vx = dwread_epd(unitEpd + 0xF8/4);
    vx = f_dwread_epd(unitEpd + 0xF8 // 4)
    # (Line 5) const vy = dwread_epd(unitEpd + 0xFC/4);
    vy = f_dwread_epd(unitEpd + 0xFC // 4)
    # (Line 7) return vx, vy;
    EUDReturn(vx, vy)
    # (Line 8) }
    # (Line 10) function setVxy(unitEpd, vx, vy)

# (Line 11) {
@EUDFunc
def f_setVxy(unitEpd, vx, vy):
    # (Line 12) dwwrite_epd(unitEpd + 0xF8/4, vx);
    f_dwwrite_epd(unitEpd + 0xF8 // 4, vx)
    # (Line 13) dwwrite_epd(unitEpd + 0xFC/4, vy);
    f_dwwrite_epd(unitEpd + 0xFC // 4, vy)
    # (Line 14) }
    # (Line 16) function renderUnit(unitEpd, option)

# (Line 17) {// move unit
@EUDFunc
def f_renderUnit(unitEpd, option):
    # (Line 18) const vx, vy = getVxy(unitEpd);
    vx, vy = List2Assignable([f_getVxy(unitEpd)])
    # (Line 19) if(vx != 0 || vy != 0)
    if EUDIf()(EUDSCOr()(vx == 0, neg=True)(vy == 0, neg=True)()):
        # (Line 20) {
        # (Line 21) const unitX, unitY = utilEud.getUnitXY(unitEpd);
        unitX, unitY = List2Assignable([utilEud.f_getUnitXY(unitEpd)])
        # (Line 22) const unitPos = (unitX + vx) + (unitY + vy)*65536;
        unitPos = (unitX + vx) + (unitY + vy) * 65536
        # (Line 23) utilEud.moveLocationXY($L('locPre'), unitX, unitY);
        utilEud.f_moveLocationXY(GetLocationIndex('locPre'), unitX, unitY)
        # (Line 24) utilEud.moveLocationXY($L('locDst'), unitX + vx, unitY + vy);
        utilEud.f_moveLocationXY(GetLocationIndex('locDst'), unitX + vx, unitY + vy)
        # (Line 25) MoveUnit(1, utilEud.getUnitType(unitEpd), utilEud.getPlayerID(unitEpd), $L('locPre')+1, $L('locDst')+1);
        ...
        if EUDIf()(option):
            # (Line 27) {
            # (Line 29) bwrite_epd(unitEpd + 0x021 / 4,  0x021 % 4, (atan2(vy, vx)*256/360 + 64)%256);
